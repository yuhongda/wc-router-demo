...
export function hasChildChecked(item: TreeNode, curValue: ValueType[]): boolean {
  function dfs(node: TreeNode): boolean {
    ...
    if (curValue.map((v) => v.slice(-1)[0]).includes(value)) {
      return true;
    }
    if (!children) {
      return false;
...
export function hasParentChecked(item: TreeNode, value: ValueType[]): boolean {
  ...
  while (tmp) {
    if (value.map((v) => v.slice(-1)[0]).includes(tmp.value)) {
      return true;
    }
...
// 状态提升
export function liftTreeState(item: TreeNode, curVal: ValueType[], depth: number): ValueType[] {
  const { value } = item;
  if (depth == 2) {
    return [...curVal, [item.parent?.parent?.value || '', item.parent?.value || '', item.value]];
  } else if (depth == 3) {
    return [
      ...curVal,
      [
...
export function sinkTreeState(root: TreeNode, value: ValueType[]): ValueType[] {
  ...
  return value.filter((v) => v.slice(-1)[0] != root.value);
}

// checked, unchecked 时重新计算
export function reconcile(item: TreeNode, checked: boolean, value: ValueType[], depth: number): ValueType[] {
  if (checked) {
    ...
    //   return value;
    // }
    return liftTreeState(item, value, depth);
  }
