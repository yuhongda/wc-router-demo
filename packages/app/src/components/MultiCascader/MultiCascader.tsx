  ...
  React.forwardRef((props: Props, ref) => {
    ...
    useImperativeHandle(
      ...
      () => {
        return {
          // 匹配所有叶子节点的 value
          matchAllLeafValue: (v: ValueType[]) => matchAllLeafValue(v, flattenData),
        };
      },
      [flattenData]
    );
    ...
    const handlePopUpLayout = useCallback(
      (popup: HTMLDivElement) => {
        if (popupVisible) {
          setPopupPlacement(getPopupPlacement(selectorRef.current, parseInt(getComputedStyle(popup).height, 10)));
        }
      },
      [popupVisible]
    );
    ...
    return (
      <Trigger
        ...
        forceRender
        key={updatePopupKey}
        popup={<Popup {...props} onCancel={handleCancel} onConfirm={handleConfirm} onLayout={handlePopUpLayout} />}
        ...
        {...popupPlacement}>
        <Selector forwardRef={selectorRef} onRemove={handleItemRemove} onClear={handleClear} {...props} />
      </Trigger>
    );
  })
);

const MultiCascader: React.FunctionComponent<Props> = React.forwardRef((props: Props, ref) => {
  return (
